...
public class BufferedStream {
    ...
    private static void bufferFileWriter() throws IOException {
        DataOutputStream out = new DataOutputStream(new BufferedOutputStream(new FileOutputStream("test_buffer.txt")));
    ...
    private static void fileWriter() throws IOException {
        DataOutputStream out = new DataOutputStream((new FileOutputStream("test_buffer.txt")));
        for (int i = 0; i < BYTES_NUMBER; i++) {
            out.writeChar(SYMBOL);
        }
        out.close();
